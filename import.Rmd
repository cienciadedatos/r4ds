...
En este capítulo aprenderás cómo cargar archivos planos en R con el paquete __readr__, el cual es parte de los archivos base de **tidyverse**.
...
El primer argumento de `read_csv()` es el más importante: es la ruta hasta el archivo a leer.

```{r, message = TRUE}
alturas <- read_csv("data/alturas.csv")
```

Cuando ejecutas `read_csv()`, devuelve una especificación columnar que indica el nombre y tipo de cada columna. Esta es una parte importante de **readr**, sobre la cual volveremos luego en [análisis estrutural de un archivo]. 

También puedes adicionar un archivo csv linear. Esto es útil para experimentar con **readr** y para crear ejemplos reproducibles para ser compartidos.
...
1. A veces hay unas pocas líneas de metadata al comienzo del archivo. Puedes usar `skip = n` para saltear las           primeras `n` líneas: o usar `comment = "#"` para quitar todas las líneas que comienzan con, por ejemplo, `#`.
    ...
    read_csv("# Un comentario que quiero saltear
      x,y,z
      1,2,3", comment = "#")
    ```
...
Si utilizaste R anteriormente, tal vez te preguntas por qué no usamos `read.csv()`. Hay unas pocas y buenas razones para preferir las funciones de **readr** sobre los equivalentes de base:

* Generalmente son mucho más rápidas (~10x) que sus equivalentes de base. Los trabajos que tienen un tiempo de corrida prolongado poseen una barra de progreso, así puedes ver qué está ocurriendo. Si buscas la velocidad bruta, prueba `data.table::fread()`. No se ajusta tan bien en **tidyverse** pero puede ser bastante más rápido.
...
Antes de entrar en detalles sobre cómo **readr** lee archivos del disco, necesitamos desviarnos un poco para hablar sobre las funciones `parse_*()`. Estas funciones toman un vector de caracteres y devuelven un vector más especializado, como un vector lógico, numérico, o fecha:
...
Estas funciones son útiles por sí mismas, pero también son un bloque estructural importante para **readr**. Una vez que aprendas cómo funcionan los segmentadores individuales en esta sección, volveremos atrás y veremos cómo se combinan entre ellos para segmentar un archivo completo en la próxima sección.

Como todas las funciones en **tidyverse**, las funciones `parse_*()` son uniformes: el primer argumento es un vector de caracteres a segmentar, y el argumento `na` especifica qué cadenas deberían ser tratadas como faltantes: 
...
Si hay muchas fallas de segmentación, necesitarás utilizar `problems()` (del inglés, problemas) para obtener la totalidad de ellas. Esto devuelve una tibble que puedes luego manipular con **dplyr**.


```{r}
problems(x)
```
Utilizar los segmentadores es más que nada una cuestión de entender qué está disponible y cómo trabajan con diferentes tipos de entradas. Hay ocho segmentadores particularmente importantes. 

1.  `parse_logical()` y `parse_integer()` segmentan valores lógicos y números enteros respectivamente.No hay prácticamente nada que pueda salir mal con estos segmentadores así que no los describiré en más detalle aquí.

1. `parse_double()` es un segmentador numérico estricto, y `parse_number()` es un segmentador numérico flexible. Son más complicados de lo que podrías esperar porque diferentes partes del mundo escriben los números en diferentes formas.
...
1.  `parse_datetime()`, `parse_date()`, y `parse_time()` te permiten segmentar diversas especificaciones de fechas y horas. Estos son los más complicados porque hay muchas formas diferentes de escribir fechas. 

Las secciones siguintes describen estos segmentadores en mayor detalle.
...
Para responder al primer problema, **readr** tiene el concepto de "locale", un objeto que especifica las opciones de segmentación que difieren de un lugar a otro. Cuando segmentamos números, la opción más importante es el caracter que utilizas como símbolo decimal. Puedes sobreescribir el valor por defecto de `.` creando un nuevo locale y estableciendo el argumento `decimal_mark` (del inglés, "símbolo decimal"):
...
Las codificaciones son un tema rico y complejo, y solo te di un pantallazo aquí. Si quisieras aprender más al respecto, te recomendaría que leas la explicación detallada en <http://kunststube.net/encoding/>.
...
Debes elegir entre tres segmentadores dependiendo si quieres una fecha (el número de los días desde el 01-01-1970), una fecha-hora (el número de segundos desde la medianoche del 01-01-1970), o una hora (el número de segundos desde la medianoche). Cuando se llaman sin argumentos adicionales: 
...
La mejor manera de deducir el formato correcto es crear unos pocos ejemplos en un vector de caracteres, y probarlos con una de las funciones segmentadoras. Por ejemplo: 
...
Ahora que aprendiste cómo segmentar un vector individual, es tiempo de volver al comienzo y explorar cómo **readr** segmenta un archivo. Hay dos cosas nuevas que aprenderás al respecto en esta sección: 
...
1.  La columna podría contener muchos valores ausentes. Si las primeras 1000 filas contienen solo `NA`s, **readr** deducirá que es un vector de caracteres, mientras que tu probablemente quieras segmentarlo como algo más específico.
...
```{r}
desafio <- read_csv(readr_example("desafio.csv"))
```

(Nota el uso de `readr_example()` que encuentra la ruta a uno de los archivos incluidos en el paquete)

Hay dos resultados impresos en la consola: la especificación de columna generada al mirar las primeras 1000 filas, y las primeras cinco fallas de segmentación. Siempre es una buena idea extraer explícitamente los `problemas()`, así puedes explorarlos en mayor profundidad:
...
Una buena estrategia es trabajar columna por columna hasta que no queden problemas. Aquí podemos ver que hubieron muchos problemas de segmentación con la columna `x` - hay caracteres arrastrados luego del valor entero. Esto sugiere que necesitamos utilizar un segmentador doble en su lugar.
...
*   En el ejemplo previo, simplemente fuimos desafortunados: si miramos solo una fila más que el número por defecto, podemos segmentar correctamente de una sola vez:
    ...
    ```{r}
    desafio2 <- read_csv(readr_example("desafio.csv"), guess_max = 1001)
    desafio2
    ```
...
*   Algunas veces es más fácil diagnosticar problemas si lees todas las columnas como vectores de caracteres:
    ...
    ```{r}
    desafio2 <- read_csv(readr_example("desafio.csv"), 
      col_types = cols(.default = col_character())
    )
    ```
    
    Esto es particularmente útil en combinación con 'type_convert()', que aplica la heurística de segmentación a las columnas de       caracteres en un marco de datos.
...
*   Si tienes problemas de segmentación importantes, a veces es más fácil leer simplemente un vector de caracteres de líneas con `read_lines()`, o incluso un vector de caracteres de largo 1 con `read_file()`. Luego puedes utilizar las habilidades sobre segmentación de cadenas de texto que aprenderás más adelante para segmentar formatos más exóticos.
